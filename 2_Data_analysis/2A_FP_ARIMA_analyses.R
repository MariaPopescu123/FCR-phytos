...
FP_AR_NAs <- read_csv("./2_Data_analysis/FP_megamatrix.csv")%>%
  select(-Chem_Depth_m,-Temp_Depth_m)
FP_AR_NAs <- FP_AR_NAs[c(24,47,68),]

CS_AR_NAs <- read_csv("./2_Data_analysis/CS_megamatrix.csv")%>%
  select(-Chem_Depth_m,-Temp_Depth_m)
CS_AR_NAs <- CS_AR_NAs[c(24,47,68),]

my.fp.data <- bind_rows(my.fp.data, FP_AR_NAs)%>%
  arrange(Date)
my.cs.data <- bind_rows(my.cs.data, CS_AR_NAs)%>%
  arrange(Date) %>%
...
#get rid of Cmax_DOC_mgL
#get rid of DINmax_ugL 
#get rid of thermo.depth

mydata2 <- mydata %>%
  select(-Year, -Cmax_SRP_ugL, -Cmax_DOC_mgL, -DINmax_ugL, -thermo.depth)
...
write.csv(final, "./2_Data_analysis/arima_results.csv",row.names = FALSE)
...
cols <- c(2:13)
response.vars <- colnames(mydata4)[14:26]
top.models <- NULL
final <- read.csv("./2_Data_analysis/arima_results.csv") %>%
...
write.csv(top.models,"./2_Data_analysis/top_models.csv",row.names = FALSE)
